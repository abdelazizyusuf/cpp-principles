...
2. **دالة main():** هي الدالة الرئيسية في البرنامج، حيث يبدأ تنفيذ البرنامج وتشير int في بداية الدالة إلى أن الدالة سترجع قيمة صحيحة في حين يدل القوسان الفارغان إلى أن هذه الدالة لا تأخذ أي متغيرات.
3. **return 0:**
 هذه العبارة تنهي الدالة الرئيسية ويُرجع قيمة 0 والتي تعني أن البرنامج قد نفذ بنجاح.
...
المتغير هو شيء يشبه الحاوية، حيث يتم تخزين أي شيء فيه، وقد تتغير قيمة المتغير  أثناء تنفيذ البرنامج وفي لغة C++ يجب عليك أن تعلن عن المتغير أولاً وبعد ذلك تقوم بتهيئته
...
1. **عدم وجود مسافة بين كلمتين:** فلا يمكن وضع مسافات بين كلمات مكونة لاسم المتغير. على سبيل المثال، "firstName" بدلاً من "first name".

2. **عدم استخدام أحرف خاصة ماعدا الشرطة السفلية _:**  يمكنك فقط استخدام الشرطة السفلية _ كحرف خاص ضن أسماء المتغيرات. مثال: "name_of_student".

3. **استخدام مزيج من المحارف:**
   - يمكن استخدام مزيج من الأحرف الأبجدية والأرقام في اسم المتغير. مثال: "value2".

4. **يمكن استخدام الأرقام:** يُسمح بوجود الأرقام في أسماء المتغيرات. مثال: "anything123".

5. **تجنب استخدام كلمات محجوزة:** لا يمكن استخدام كلمات محجوزة في لغة البرمجة كأسماء للمتغيرات. مثال لا يمكن استخدام "if" أو "while" كأسماء متغيرات لأن لها معاني خاصة في اللغة كما سنتعلم في دروس لاحقة.

6. **يجب ألا يبدأ اسم  المتغير برقم:** لا يجوز أن يبدأ اسم المتغير برقم. مثال: "123num".

7. **أسماء المتغيرات حساسة لحالة الأحرف:** يعتبر الاسم حساسًا لحالة الأحرف، مما يعني أن "num" و "Num" يُعتبران متغيرين مختلفين.
...
يمكن كتابة التعليقات على سطر واحد حيث تبدأ تعليقات على سطر واحد بـ "//" وتستمر حتى نهاية السطر.

```cpp
// هذا تعليق على سطر واحد
```

تكما يمكن كتابة عليقات متعددة الأسطر إذ تبدأ التعليقات متعددة الأسطر بـ "/" وتنتهي بـ "/". كل ما يتواجد بين هذين الرمزين يُعتبر تعليقًا.
...
يُشير نوع البيانات Data Type في لغة البرمجة ++C إلى تصنيف البيانات ويُخبر مترجم اللغة بكيفية تفسيرها والتعامل معها.
...
فيما يلي أمثلة على استخدام بعض الدوال الرياضية من مكتبة `<cmath>` في لغة البرمجة ++C:
...
مثال على إنشاء سلسلة نصية وطباعة طولها أو عدد حروفها
...
مثال على دمج سلسلتين في سلسلة واحدة
```cpp
string str2 = "Hello";
    string str3 = "World";
...
مثال على الوصول إلى حرف معين في سلسلة نصية
```cpp
string str4 = "Example";
...
**التعليمة if:** تستخدم لتنفيذ الكود إذا كانت شرط معين مُحققًا.
...
**التعليمة if-else:** تستخدم لتنفيذ كود أول إذا كانت شرط معين مُحققًا، وكود مختلف إذا كان الشرط غير صحيح
...
**التعليمة if-else if-else:**تستخدم لفحص شروط متعددة وتنفيذ مجموعات مختلفة من الأكواد اعتمادًا على أي شرط هو صحيح.
...
### الحلقات التكرارية: تستخدم الحلقات في البرمجة لتكرار جزء من التعليمات البرمجية عدة مرات وتتضمن لغة ++C عدة أنواع من الحلقات وهي
...
1.
 1. بالتحكم في استخدام الذاكرة بشكل دقيق، مما يسهم في تحسين كفاءة استخدام الموارد.
 2.  تبادل ونقل عناوين الذاكرة بين المتغيرات والدوال.
 3.  تحسين أداء البرنامج خاصة عند التعامل مع كميات كبيرة من البيانات.
 4.  تسهل التعامل مع هياكل البيانات مثل المصفوفات والسلاسل النصية.
 5.  يجعل تبادل البيانات مع الدوال أكثر كفاءة.
 6.  فهم المؤشرات يصبح أمرًا ضروريًا في المستويات المتقدمة من البرمجة لتحقيق برامج فعالة ومتقدمة.
...
1. **نمط القيمة المعادة (Return Type):** يحدد نوع القيمة التي ستعيد الدالة. يمكن أن يكون أي نوع بيانات، مثل الأعداد الصحيحة (`int`) أو الأعداد العشرية (`double`)، أو حتى لا يكون هناك قيمة (`void`) في حالة عدم إرجاع قيمة.

2. **اسم الدالة (Function Name):** يحدد اسم الدالة الذي يستخدم لاستدعائها في أماكن أخرى في الشيفرة.

3. **قائمة البارامترات (Parameter List):** تحتوي على المتغيرات التي يمكن للدالة استخدامها والتي يجب تمريرها عند استدعاء الدالة. 

4. **جسم الدالة (Function Body):** يحتوي على مجموعة الشيفرة التي تشكل تنفيذ الدالة. يتم تحديد هذا المجموع بين الأقواس السعفية '{ }'.
